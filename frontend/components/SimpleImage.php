<?php
    ...
    class SimpleImage
    {
        ...
        function complect($filename, $savepath, $lists = array())
        {
            if (!file_exists($savepath)) @mkdir($savepath, 0777, true);
            if (in_array("sm", $lists)) $this->load($filename)->fit_to_width(200)->save($savepath . 'sm.jpg');
            exit("asas");

            if (in_array("lg", $lists)) {
                if ($orgW > 800) $this->load($filename)->fit_to_width(800)->save($savepath . 'lg.jpg');
                else $this->load($filename)->resize(($orgW - 1), ($orgH - 1))->save($savepath . 'lg.jpg');
            }
            // $this->load($filename)->resize(($orgW-1), ($orgH-1))->save($savepath.'mx.jpg');
            @unlink($filename);
        }
        ...
        function overlay($overlay, $position = 'center', $opacity = 1, $x_offset = 0, $y_offset = 0)
        {
            ...
            switch (strtolower($position)) {
                case 'random':
                    $x = 0 + rand(0, $this->width - $overlay->width);
                    $y = 0 + rand(0, $this->height - $overlay->height);
                    break;
                case 'top left':
                    $x = 0 + $x_offset;
                    $y = 0 + $y_offset;
